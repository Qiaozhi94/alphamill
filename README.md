...
北极星是**在跑组合的持续盈利**（ADR-0008）：所有在跑组合合并资金曲线的成本后净超额收益（相对
各策略预注册基准）——滚动 90 天为正、最近 4 个滚动窗至少 3 个为正、回撤不破预注册阈值、
单组合达到可信样本量。满足判据的在跑产出计为 **可信 Alpha**：有效独立、成本后为正、通过
最终确认与无前视审计、部署前仍有效，并对现有组合有正边际贡献。

不设周期产出配额：名义候选数只作漏斗分母与能力标定；有效独立数、完整漏斗、来源多样性、
研究周期和线上偏差作为护栏与领先指标一起报告。
