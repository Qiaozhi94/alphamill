...
- manifest 合成是**累计完整快照**而非一日 delta：上一 valid 清单全量继承为基线，
  本轮分区按逻辑分区键整项替换/追加，`rows`/`pairs`/quality 重算，`skipped`
  走同样的继承/替换（F002-R2-03/R3-03）。
"""
...
from alphamill.data_bridge.registry import DatasetSpec

DATA_VERSION_RE = re.compile(r"^v(\d{4})\.(\d{2})\.(\d{2})(?:-r(\d+))?$")
...
def parse_data_version(version: str) -> tuple[date, int]:
    m = DATA_VERSION_RE.match(version)
    if m is None:
...
def validate_manifest_integrity(root: Path, manifest: dict[str, Any]) -> None:
    """逐项校验清单内文件存在、字节数相符、sha256 相符（只按清单，不扫目录）。"""
...
def publish_manifest(root: Path, manifest: dict[str, Any]) -> Path:
    """原子发布：临时名写入后 rename；写成功前该 data_version 对 reader 不存在。"""
