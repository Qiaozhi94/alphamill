...
| Kronos 服务生命周期端点 | 架构 §7.1 Kronos 服务生命周期契约（`status`/`stop`/`restore`、`contract_version`、错误码） | 夜槽编排前置（F003 开工前落地） | 待分配 | 契约正文见架构 §7.1；F003 客户端已 fail-closed 兜底，端点缺失不阻塞 F003 的接口验收，但夜槽实测需端点就绪 |
