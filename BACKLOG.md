...
| F008 后续：导出清单绑定**当前宇宙版本**（落选即移出） | FR-006、spec §3 边界场景 / §5 状态机 / `Q-003` | 待排 | 待分配 | 现状：`export_admitted` 取「每 pair 最新判定」且 `universe_id=None`，旧宇宙的 ACTIVE 判定永久有效 → 「跌出阈值但仍在交易」的 pair 在新版 `UniverseDef` 中落选后**仍留在导出清单**；spec 要求的状态机（落选 → 移出导出清单、台账不动）没有任何代码路径，唯一出口是退市 `valid_to` 或新 QUARANTINED 判定。修法：导出入口带当前 `universe_id`，把准入集合与新定义 `selected` 求交。证据：`docs/reviews/RETROSPECTIVE.md` 循环 21 · R1-016） |
