...
- **真实环境测试纪律**：需要真实进程/文件系统/端到端/GPU 的测试一律在**能力所在的机器**上
  直接执行，不允许默认标记成「待用户在真实环境验证」而跳过。唯一允许延后的情形是客观
  不可执行（缺凭证、缺二进制、需外部账号），此时必须在自检结论里显式列出原因与补齐方式。
  Feature 收口前，集成验收必须运行
  `ALPHAMILL_INTEGRATION=1 python3 -m pytest tests/integration -q`；服务不可达或
  mock 模式在该开关下必须判红，不能以 skip 代替证据。
- **机器边界**（拓扑见 `docs/alphamill-architecture.md` §7.1）：开发机无 GPU，只承载编码、
  单元测试与 `python3 tools/verify.py`；**集成与 GPU 证据一律在执行机取**（同一时刻只有一台
  执行机：当前 `qiaozhi-lt`，成熟后整体迁移至 `qiaozhi-lab`）。开发机上的 skip 不是证据，
  也不是失败——它只说明"这台机器没有这个能力"；验收证据必须记录取证机器的 hostname 与设备
  （GPU 型号 / `device=cpu`），不得把开发机的 CPU 结果当作执行机结论。执行机迁移后，依赖
  显存预算、时段表或性能的验收必须在新机重跑，不得继承旧机结论。
