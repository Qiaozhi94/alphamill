...
AlphaMill 按四个平面组织。AI 控制面提出候选和建议，证据面独立裁决，交易面只执行已批准的
版本化策略，数据面提供可复现输入。外部工具必须接在稳定契约之后，不能成为跨层捷径。
...
┌──────────────▼─ 证据与治理面 ─────────────────────────────────────────────┐
│ 因子评测/去重 → 选择期 → 冻结 PortfolioDef → 留出 → 最终确认             │
│ experiment ledger 串联数据、代码、因子、组合、信号、审批与运行结果       │
└──────────────┬───────────────────────────────────────────────▲────────────┘
               │ PortfolioDef / DeploymentCandidate           │ 成交与绩效
┌──────────────▼─ 策略与执行面 ─────────────────────────────────────────────┐
│ 组合门 → Top-K/权重/净额化/风险预算 → 版本化 signal_cache → Freqtrade    │
