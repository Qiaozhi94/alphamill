...
**时段表的时刻一律是执行机本地时间**（「工作日夜」「周末白天」这两行本身就需要本地日历才能读）；时区随执行机走，由挖掘配置的 `training_window_tz` 承载（默认 `Asia/Shanghai`），迁移执行机时只改配置不改代码。落盘时间戳仍一律 UTC——只有时段判定是时区感知的。
...
| 动作 | 语义 | 幂等性 | 超时 | 错误码 |
|---|---|---|---|---|
| `status` | 返回 `{state: running\|stopped\|transitional, desired: running\|stopped, contract_version, model_loaded, vram_bytes, vram_readable, device, operation}` | 只读，天然幂等 | 服务端处理 deadline，可配（默认 5s） | `E_UNSUPPORTED_VERSION` |
| `stop` | 置期望态为 `stopped`、卸载模型并释放显存，返回释放后的 `vram_bytes` | 重复调用返回 `state=stopped`，不报错 | 服务端动作 deadline，可配（默认 60s） | `E_BUSY` / `E_TIMEOUT` / `E_UNLOAD_FAILED` / `E_BAD_REQUEST` / `E_UNSUPPORTED_VERSION` |
| `restore` | 置期望态为 `running`、恢复常驻推理，返回 `state=running` | 重复调用返回 `state=running` | 服务端动作 deadline，可配（默认 120s） | `E_BUSY` / `E_TIMEOUT` / `E_UNAVAILABLE` / `E_BAD_REQUEST` / `E_UNSUPPORTED_VERSION` |
...
- **期望态与派生态**：`state` 不是"模型此刻在不在内存里"的同义词，而是 `(desired, model_loaded)`
  ...
  model_loaded=true`，两者之外为**过渡态，其 wire 值就是 `state: "transitional"`**（`operation`
  同时非空）。动作一旦受理就先置 `desired`、后改 `model_loaded`，因此过渡态**必然出现在
  每次 stop/restore 的执行窗口内**，不是异常分支：`state` 的取值域若只有两个，服务端在这段
  窗口里根本构造不出合法响应。客户端见到 `transitional` 一律 **fail-closed**——既不得当作
  已停机去取锁，也不得当作在跑而跳过卸载。**`desired=stopped` 期间推理端点
...
- **失败落点（逐注入点）**：动作抛错时，服务端**必须先把 `operation` 清空、把
  `(desired, model_loaded)` 收敛成一个稳定态，然后才返回错误信封**——不允许"返回了错误但
  状态停在过渡态"。`stop` 的三个注入点各有确定落点：卸载**尚未开始**即失败 → 回落
  `desired=running`（原状不变），返回 `E_UNLOAD_FAILED`；模型引用**已丢弃**之后失败（含
  `empty_cache()` 抛错）→ 不回滚，保持 `desired=stopped` 且 `model_loaded=false`，仍返回
  `E_UNLOAD_FAILED`，`status.vram_bytes` 如实反映显存可能未回收，编排据"未释放"fail-closed。
  `restore` 加载失败 → `desired` 回落 `stopped`，返回 `E_UNAVAILABLE`，进程不退出。
  判据：丢引用不可逆，**不可逆的一步之后只准前进不准假装回滚**。
...
- **status 的 deadline 归属**：动作表里 status 的 5s 是**服务端处理 deadline**，不是客户端
  的 socket 超时。显存探测（`torch.cuda.mem_get_info` / `nvidia-smi` 子进程）只能在该 deadline
  的**剩余预算**内执行，探测超时即按"读数不可得"返回成功响应，**不得**让一次卡住的
  `nvidia-smi` 把 status 拖过契约 deadline——status 必须在任何状态下都可达，这是控制面的
  基本承诺。探测顺序（torch → nvidia-smi → 不可得）可由服务端配置固定为单一来源，
  变量名、值域与非法值策略见 F009 FR-007；
- **显存确认**：`stop` 之后编排必须经 `status` 的 `vram_bytes` 与设备侧读数确认显存已释放，
  未确认不得取锁训练；**"已释放"的判据是读数真实下降，且卸载后整卡可用显存达到训练预算**
  （与单槽取锁同一阈值 `vram_limit_gb`，不另立第二份配置）——仅凭 `vram_bytes` 是合法整数
  不构成确认。判据落在**可用侧**而不是"已用读数低于预算"：8GB 卡上"已用 5GB"同时满足
  "低于 6GB 预算"和"取不到 6GB"，夜槽要判的是后者。`vram_readable=false` 是"读数缺失"，
  与"确实没释放"必须分别记账——两者同为 fail-closed，但事后归因完全不同；
- **客户端超时**：编排必须按动作分别设置超时，不得对三个动作共用单一超时——用 `status`
  的超时去卡 `stop` 会把正常卸载误判成失败。客户端的 socket deadline **必须严格大于**同一
  动作的服务端 deadline（缺省为服务端值 + 5s 余量，可配）：两端取同一个数值时，网络与
  序列化开销会让客户端先超时抛 `OSError`，服务端规范的 `E_TIMEOUT` 信封根本收不到，
  "动作仍在进行"这条语义就等于没有；
- **迟到副作用的恢复所有权**：`E_TIMEOUT` 表示动作仍在进行，因此**一旦发出过 `stop`，
  客户端即持有恢复责任**——无论该请求返回成功、`E_TIMEOUT`、其他错误还是连接中断，
  编排在离开训练窗口前都**必须**调用一次 `restore`（幂等，未停机时返回 `running` 无副作用）。
  只在"同步确认 stopped"时才登记恢复，会让 60s 超时返回、61s 后台完成的那次卸载永久停机，
  白天 dry-run 从此拿不到实时信号。客户端可另以 `status.operation` 转 `null` 观察最终落点，
  但恢复责任**不依赖**轮询成功——轮询窗口之外的迟到完成同样要被 `restore` 兜住；
...
- **观测 → 处置决策表**：「服务确未部署」等判定只依据可观测行为与编排自身的部署清单
  ...
  | `status.state=transitional`（动作进行中） | 状态未知 | **fail-closed** 留在单槽队列，不得当作已停机 |
  | `stop` 返回 `E_BUSY` / `E_TIMEOUT` / `E_UNLOAD_FAILED`，或 `status.vram_bytes` 确认未释放 | 停止失败 | **fail-closed** 留在单槽队列；已发出过 `stop` 即持有恢复责任，离开前 `restore` |
