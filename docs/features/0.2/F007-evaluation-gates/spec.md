...
- 因子注册表的**定义面**（`FactorDef` 内容寻址读写与定义版本）→ F003；本 Feature 是**评测摘要回写、`|ρ|`（`>0.99` 拒绝 / `0.90~0.99` 标记变体）查重判定与 lifecycle 状态判定**的唯一写入 owner（PRD FR2.5 的评测面），lifecycle 动作执行归 F006。
