...
**没有 point-in-time 宇宙**——今天的"宇宙"是库内 `DISTINCT symbol` 的隐式结果，只反映**现在**有哪些 pair，不回答"2025-03-01 那天有哪些 pair 可交易"。用当前成员表回填历史就是幸存者偏差：退市的、后来才上市的 pair 会被错误地当成全程存在。架构 §4.1.1 明确要求横截面算子只在同一 timestamp 的 point-in-time 宇宙内计算，`F007` 的 ResearchSnapshot 也要绑定 universe 摘要——这两处现在都没有可消费的真相源。
...
作为项目所有者，我希望 4600 万行的回填能限速、能断点续跑、能看到进度，以便这件 1~2 周的事不会因为一次中断就从头再来。
...
- `universe_membership` 只追加台账（联机库）与 `universe_at(T)` 查询；按 `symbol_map` 同构的方式发布为**内容寻址 artifact** 到湖（ADR-0007 与 `F007` IR-002 都按 artifact 定义它）；
...
- pair 退市或跌出流动性阈值：历史数据保留、台账记退出时间与原因、导出清单移除；**不删数据**（删了就制造幸存者偏差）。
...
系统应当分批回填目标宇宙的历史数据，遵守交易所限速并在限流错误上退避重试（重试次数有上限）；回填应当幂等、可中断、可从断点续跑，且逐 pair 的进度与失败原因可查。
...
系统应当维护只追加的宇宙成员台账，每条记录带生效区间与进出原因；系统应当提供 `universe_at(T)` 查询，返回 T 时刻的成员集合。历史区间一经发布不得修改。
...
当 pair 通过质量门时，系统应当把它纳入 `F002` 的导出清单；宇宙台账应当以 `symbol_map` 同构的方式发布为内容寻址 artifact（canonical 字节的 SHA-256 为 digest，同 digest 文件必须逐字节一致），供 ResearchSnapshot 按显式 digest 引用。`symbol_map` 因新 pair 产生新 digest 时，旧 digest 应当仍可读取。
...
- **DR-002**：`universe_membership` 应当以 `(exchange, market_type, db_symbol, lake_pair, valid_from, valid_to, reason, universe_id)` 表达成员区间；`valid_to` 为空表示当前有效；**只追加，历史区间不可改写**。联机库持有可查询的真相源，湖内持有其内容寻址快照 artifact。
- **DR-003**：`BackfillRun` 应当记录目标 pair 集合、窗口、限速参数、逐 pair 进度与行数、重试与失败原因、断点位置、起止时间与 hostname。
- **DR-004**：质量门结果应当持久化为逐 pair 的判定记录（各项指标值、阈值、verdict、原因码），失败记录与通过记录同样保留。

### 事件 / Trace 需求

- **TR-001**：成员进出时，系统应当写入 `universe.member_changed`，payload 包含 pair、方向（进/出）、生效时间、原因与 `universe_id`。
...
- **IR-002**：宇宙台账 artifact 应当以固定 canonical 格式（列序、排序、UTF-8、LF）发布到湖元数据目录，路径按 digest 寻址；读取接口应当支持按 digest 加载并提供 `universe_at(T)` 语义查询。
- **IR-003**：台账与运行记录应当带 `schema_version`，供 `F003`（横截面 PIT 掩码）与 `F007`（ResearchSnapshot 的 universe 摘要）只读消费。
...
- **NFR-003**：正确性：`universe_at(T)` 对任意 T 返回的成员集合不含 T 之后才进入的 pair，也不排除 T 时在册但之后退出的 pair（无幸存者偏差）。
- **NFR-004**：机器边界：回填、质量门与导出都在执行机执行（架构 §7.1）；开发机只跑单元与契约测试，运行记录必须标注 hostname。
- **NFR-005**：容量：40 对的数据量约 4200 万行（按 F002 基线外推：约 29,300 个 ohlcv 分区、全量导出约 25min、NAS 约 58,800 个分区文件）；磁盘占用、单次全量导出耗时与 NAS 备份时长必须实测记录，与 6 对基线（631 万行、全量 3m48s、NAS 8,823 文件）对照。**实测显著劣于外推时必须报告，不得默认可接受**。
...
ACTIVE      -> RETIRED        退市或跌出流动性阈值；写 valid_to 与原因，移出导出清单
                              （历史数据保留，不删）
...
- [ ] **AC-005** (`FR-004`, `DR-004`): 缺失率超限、未闭合边界、连续聚合对不上三类 fixture 均被拦在导出清单外并各记原因码；全项通过者进入清单 — tests: `tests/integration/test_f008_quality_gate.py`
...
- [ ] **AC-009** (`FR-006`, `IR-002`): 台账 artifact 同内容得同 digest 且逐字节一致、内容变化得新 digest 且旧 digest 仍可读；新 pair 进入导出清单后 symbol_map 同样满足该性质 — tests: `tests/integration/test_f008_export_integration.py`
- [ ] **AC-010** (`TR-001`, `TR-002`, `DR-003`, `NFR-004`): 成员变更与回填进度/失败事件可按 run 与 pair 查询；运行记录标注 hostname — tests: `tests/integration/test_f008_backfill.py`
...
- 外部 / 环境依赖：Binance 公开行情与限流策略；执行机的磁盘余量（约 4600 万行）；NAS 备份窗口。
...
| 退市 / 跌出阈值的 pair 怎么处理 | 历史数据永久保留，台账记 `valid_to` 与原因，仅移出导出清单 | 删数据等于制造幸存者偏差，与 FR1.5 的立意直接冲突 | 退出原因进入 `universe.member_changed` |
...
| 台账是新 dataset 还是 artifact | **artifact**：按 `symbol_map` 同构内容寻址发布，不进 dataset registry | ADR-0007 与 `F007` IR-002 都把它定义为「universe/calendar artifact」；它是元数据不是时序数据，按日分区没有意义 | ResearchSnapshot 按显式 digest 引用，不解析 latest |
...
| 为什么不是 50 对 | 不做：50 对全量导出约 32min、NAS 约 73,500 个分区小文件，已踩在 F002「分钟级导出」承诺的边缘，换来的噪声改善只有 11% | 成本永久、收益递减 | 若 T022 实测余量充足且确有需要，按增量加 pair（台账天然支持） |
...
- [x] Q-003: 退市或跌出阈值的 pair 怎么处理？ — 决策：历史数据永久保留、台账记退出时间与原因、仅移出导出清单；删除等于制造幸存者偏差
