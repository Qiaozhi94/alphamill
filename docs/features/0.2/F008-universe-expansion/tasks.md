...
- [x] T020 (`FR-003`, `NFR-001`): 在执行机回填**批 1**（成交额前 30，含现有 6 对）——owner 主导，失败 pair 单独重跑 — verify: 批 1 的 `BackfillRun` 逐 pair `status=completed`（**已完成 2026-09-24**：run `6d85a249e8fe-b1-20260922T150054Z` 逐 pair `completed`（30/30），11 片、多次断点续跑；证据见 `reports/f008/执行机取证-T020-T023.md` §2/§3/§5/§6 与 `reports/backfill/<run_id>/run.json`）
- [x] T021 (`FR-003`, `NFR-001`): 回填**批 2**（第 31–40），复用同一套编排；批 1 已过门的 pair 不受影响 — verify: 批 2 的 `BackfillRun` 逐 pair `status=completed`（**已完成 2026-09-24**：run `6d85a249e8fe-b2-20260924T081842Z` 10/10；证据见 `reports/f008/执行机取证-T020-T023.md` §10）
- [x] T022 (`FR-004`): 每批回填完成后立即对该批 pair 跑质量门，通过者准入、失败者隔离并记录原因；**批 1 过门即可供 `F003` 使用，不必等批 2** — verify: `pytest -q tests/integration/test_f008_quality_gate.py` + 逐 pair 判定记录（**实测结果 2026-09-24**：批 1 → 25 ACTIVE / 5 QUARANTINED（BANK/ONDO/PUMP/TUT `missing_ratio_exceeded`、TRUMP `boundary_not_closed`，均为定义 `listed_at` 取自永续上市日所致的已裁决预期隔离）；批 2 → 10/10 ACTIVE。累计 35 对准入、台账 70 行、最终 artifact `sha256:05651fe0…`；证据与两个门禁缺陷的修复见 `reports/f008/执行机取证-T020-T023.md` §7/§9/§10）
- [x] T023 (`FR-006`, `NFR-005`, `AC-011`): 两批都过门后跑一次全量导出与 NAS 备份，实测磁盘占用、导出耗时与备份时长并与 6 对基线及外推值对照；**并完成准入切换**——把 `deployment/.env` 的 `SYMBOLS` 扩到已准入的 40 对后 `docker compose up -d --force-recreate data-collector`（`restart` 不重载 env），且切换前对新增 pair 跑追赶回填 `--start 2026-09-10T15:52:00Z --end <切换时刻>`（冻结窗口终点到切换时刻的洞，见 `reports/f008/执行机取证-T020-T023.md` §4） — verify: `tests/integration/test_f008_capacity_report.py` + 切换后 `ccxt_ingestor` 日志 `symbols_ok=35 symbols_failed=0`（按最终准入 35 对，非 40）+ 追赶回填的 `BackfillRun` 逐 pair `status=completed`。**执行完成（2026-09-24/25，执行机 `qiaozhi-lt`）**：追赶回填 run `6d85a249e8fe-b0-20260924T151641Z`（29 对 / 583,538 行）+ 补洞口 run `…154657Z`（841 行）；`SYMBOLS` 6 → **35 对** + `docker compose up -d --force-recreate data-collector`（`symbols_ok=35 symbols_failed=0`，滞后 2–3 分钟）；全量导出 `v2026.09.24`（**36,794,805 行 / 25,554 分区 / 860 s**，未命中 `guard_full_shrink`，`shrink_confirmed=false`）；湖 **8,860 → 30,028** 分区文件 / 272 → 1,015 MB；NAS 备份 **8,356 s**、NAS 端 parquet **30,028**（三段 md5 校验一致）；导出清单 = `universe_at` ∩ 质量门 ACTIVE 经真实库+真实湖逐对复核 **35/35 一致**。容量记录：`reports/f008/capacity-report.json`；证据：`reports/f008/执行机取证-T020-T023.md` §13（含途中三处缺陷与修复）
...
- [x] T026 (`AC-011`, `NFR-004`): 在执行机 `qiaozhi-lt` 归档容量与耗时实测证据（含 hostname），开发机跳过属预期不得以其结果替代 — verify: 执行机上 `ALPHAMILL_INTEGRATION=1 pytest -q tests/integration/test_f008_capacity_report.py` —— **已完成 2026-09-25**：执行机 `qiaozhi-lt` 上 `ALPHAMILL_INTEGRATION=1 pytest -q tests/integration/test_f008_capacity_report.py` = **4 passed**；记录带 `hostname=qiaozhi-lt`（`reports/f008/capacity-report.json`），含导出 860 s / 湖 30,028 文件 / NAS 8,356 s / NAS 30,028 文件，对外推值 1,330 s / 51,468 文件分别为 0.65× / 0.58×
...
- [x] T035: 回写 spec 验收证据、勾选验收清单、更新 `BACKLOG.md` 状态与 spec frontmatter；回写时须核对 T023 的准入切换两步（`SYMBOLS` 扩容 + 追赶回填）已执行并留证，并回看「门禁与台账改用数据命名空间上市时间」这一独立修复项（`reports/f008/执行机取证-T020-T023.md` §7，owner 裁决为单独评审）是否已立项 — verify: `python3 tools/validate_spec_lifecycle.py` —— **已完成 2026-09-25**：spec 的 14 条 AC 全部勾选并附执行机证据（§12 映射 + §13 实跑）；`BACKLOG.md` 移出活跃索引并新增 3 行 F008 后续项（导出清单绑定当前宇宙版本 / 发现口径去 ticker 启发式 / 台账库侧加固）；frontmatter 经 `sdd_status.py --advance` 流转；准入切换两步已核对留证（§13.2/§13.1）；「门禁与台账改用数据命名空间上市时间」已确认立项在 `BACKLOG.md`「规划中」（同批还有 §15 的四个后续项）；`python3 tools/validate_spec_lifecycle.py` 全过
...
- 与 `F003` 的关系：两批均已过门（T022，2026-09-24）——批 1 **25 对 ACTIVE / 5 对隔离**、批 2 **10/10 ACTIVE**，累计 **35 对已准入**（见 `reports/f008/执行机取证-T020-T023.md` §9/§10），**已满足 `F003` T035 的 ≥30 对前提**（35 ≥ 30）。
