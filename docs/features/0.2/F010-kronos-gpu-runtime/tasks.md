...
- [x] T011 (`FR-006`, `AC-009`): 移除 `tests/integration/test_f009_vram_release.py` 的 `xfail(strict=True)`，在执行机真实通过（显存下降且卸载后整卡可用显存达到训练预算）。若 T002 记录的空载可用显存已 <6GB，先按 spec §7 风险行重标 §7.1 夜槽行并在同一提交同步 F003 `vram_limit_gb` 缺省，再以重标后的预算判定。**前置**：F009 已合入主干（载体与控制面由它交付并保持先红态）；F009 不反向等待本任务（F009 检视 R4-003） — verify: `tests/integration/test_f009_vram_release.py` — 证据（2026-09-24，`qiaozhi-lt`）：`test_f009_vram_release.py` 的 `xfail(strict=True)` 已移除并真通过：`vram_before=1601699840`（1.49GB）→ `vram_after=1144520704`（1.07GB），卸载后整卡可用 **7.53GB ≥ 6GB** 训练预算，故**不触发** §7.1 夜槽行重标、F003 `vram_limit_gb` 缺省 6.0 不变。解除前先确认它在 GPU 实例上确为 XPASS（`xfail(strict)` 判红），不是凭空解除
- [x] T012 (`FR-006`): 架构 §7.1 的"GPU 基座前置"条改写为已落地，并把 F009 spec 的 NFR-006 / SC-005 的显存结论前置同步为已解除（F003 T033 只依赖 F009，不在本任务改写范围） — verify: `docs/alphamill-architecture.md` §7.1 + `python3 tools/verify.py` — 证据（2026-09-24，`qiaozhi-lt`）：架构 §7.1 的「GPU 基座前置」条已改写为「GPU 基座（已由 F010 交付）」：列出交付面（构建参数驱动的 CUDA wheel、override 集中的 GPU 面、默认 compose 不含 GPU、显式 cuda 拿不到即失败可见）与实测（`device=cuda:0`、真实信号、常驻峰值 565 MiB 在 ≤3GB 预算内→预算维持）；显存结论前置随 T011 一并解除，条文改为「控制面语义与显存释放能力均已验证」。F003 T033 只依赖 F009，不在本任务改写范围（已由对方会话于同日取证：54 passed / 0 xfailed）
...
- [x] T015 (`AC-004`): 两层证伪——单元层（开发机/CI）跑 `tests/unit/test_f010_device_strict.py` 并做删严格分支的变异判红；执行机层两例：① `docker run` GPU 镜像 `alphamill/kronos-signal-real:gpu`、**不加 `--gpus`**、`-e KRONOS_DEVICE=cuda` → 无设备分支；② 默认 CPU 镜像 + `-e KRONOS_DEVICE=cuda` → CPU wheel 分支；均断言非零退出、日志含对应文案、`/health` 不可达。**不得**以"开发机叠加 override 被守护进程拒绝"充当证据——那条路径容器从未启动，严格分支没被执行（文档检视 R1-003） — verify: `tests/unit/test_f010_device_strict.py` + 执行机上 `ALPHAMILL_INTEGRATION=1 pytest -q tests/integration/test_f010_gpu_runtime.py` — 证据（2026-09-25 重取，`qiaozhi-lt`）：单元层 16 passed（含 `cuda:1` 类显式值拒绝 4 例）+ 删严格分支变异 5 failed；**执行机两例均取到非空证据**——① GPU 镜像 `alphamill/kronos-signal-real:gpu-t033` 不加 `--gpus`、`-e KRONOS_DEVICE=cuda` → exit=3、`restarts=0`、存活期间 `/health` 不可达、日志含 `no CUDA device visible`；② CPU wheel 镜像 `alphamill/kronos-signal-real:cpu-f010`（在既有 CPU 镜像上 `pip install --no-deps` 换成当前代码，`torch 2.14.0+cpu` / `version.cuda=None`，registry 不可达时的构建法）→ exit=3、`restarts=0`、`/health` 不可达、日志含 `torch is a CPU wheel`。**此前该例曾以 skip 记账（代码检视 R1-001）**，现已用当前代码的镜像重取；compose 项目名改为按 worktree 区分，避免镜像名被并行会话覆盖
...
- [x] T017 (`AC-001`, `AC-002`, `AC-004`, `AC-005`): 运行项目统一质量门 — verify: `python3 tools/verify.py` — 证据（2026-09-24）：`python3 tools/verify.py` **978 passed / 29 skipped / 0 failed**，文档门禁与 ruff 全绿。注：首次跑出 13 个 error，复现不出来——是与另一会话并发动 8002 实例撞车（集成用例连到一半被打断），单独重跑两次均 0 error
