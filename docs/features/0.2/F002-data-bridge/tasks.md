...
- [ ] T006 (`FR-003`): `src/alphamill/data_bridge/reader.py`——DuckDB 只读取数(dataset+data_version+时间范围+pair 过滤);**返回数据前先跑 manifest 完整性校验**;`as_of=T` 同时施加 `event_time <= T` **与** `available_at <= T`(双时间轴,design §3);`realized_return_60m` 在 `evaluated_at > T` 时置 NULL 而非丢行;带质量旗分区默认抛 `FlaggedPartitionError`,`allow_flagged=True` 显式豁免并回报清单;invalid/缺失版本抛错 — verify: `tests/integration/test_f002_reader.py`(AC-003/AC-008/AC-009)
