...
- [x] T005 [P] (`FR-004`): `src/alphamill/data_bridge/symbol_map.py`——映射键 **(exchange, market_type, db_symbol)**,五列 canonical csv current 副本落 `lake/_metadata/symbol_map.csv`(集成 §2.2 权威路径)，内容寻址副本原子发布到 `lake/_metadata/symbol_maps/<digest>.csv` 并支持按 digest 加载；spot/perp 格式按 design §4 冻结，**lake_pair 碰撞直接抛 `SymbolCollisionError`** — verify: `tests/unit/test_f002_symbol_map.py`(AC-004 + 碰撞判红 + 同 symbol 跨 spot/perp 不混淆 + 旧 digest 可重放)
