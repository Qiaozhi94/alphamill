...
- `BACKLOG「Kronos 服务生命周期端点」feature 落地并部署于执行机 -> T033`：真实卸载取证以控制面端点可用为前提（T033 对 `test_f003_kronos_lifecycle.py` 要求 0 xfailed）；该载体即 F009，T033 **只依赖 F009**，不依赖 F010（GPU 推理基座）——显存真实下降断言已拆至 `test_f009_vram_release.py`（F009 T016），由 F010 AC-009/T011 解除，`kronos-signal-real` 的 CPU 实例即可作为 T033 的控制面语义取证目标；端点未落地时 T033 不可勾，取证随该载体后移，不以 xfail 先红态冒充夜槽证据。
