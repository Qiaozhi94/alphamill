...
- [ ] T001 (`Q-001`): 关闭 spec 唯一剩余的开放问题——宇宙扩容（FR1.5）是否与 F003 并行立项 — verify: `docs/features/0.2/F003-alphagen-vendor/spec.md` §8 全部 Q 关闭
...
- [ ] T003 (`FR-002`): clone 上游 AlphaGen、记录 commit hash、核对核心子集的实际文件布局，产出 vendor 清单初稿 — verify: `src/alphamill/factor_factory/generators/alphagen_vendor/VENDORED.md`
...
- [ ] T013 (`FR-002`, `AC-002`): vendor 核心子集落地（表达式/算子、张量求值器、线性协同池、RL 环境），丢弃 `requirements.txt` 与 `alphagen_qlib/`，逐处改动标注并补齐 `VENDORED.md` 五项 — verify: `tests/unit/test_f003_vendor_hygiene.py`
