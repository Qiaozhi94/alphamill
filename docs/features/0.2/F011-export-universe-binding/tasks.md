...
- [x] T006 (`FR-003`, `FR-004`, `IR-001`, `IR-002`, `IR-003`, `AC-003`, `AC-004`, `AC-005`, `AC-008`): 导出侧接线：`--universe-id`（单给即 `parser.error`）；`cli.main` 在 `_refresh_symbol_map()` 之前一次性解析绑定、固定 `window_end` 并传给全部 dataset；stderr 输出 `FATAL: <code>: <消息>`；运行摘要两键 + 逐条剔除日志；`exporter.py`/`binding.py`/`verdicts.py` ≤350 行由行数断言锁定 — verify: `tests/unit/test_f011_cli_contract.py`（red: 5ba27e6 10 failed——无 `--universe-id`、未在启动期解析、stderr 无原因码；green: CLI 契约 16 passed + F002/F008 CLI 回归 38 passed；cli.py 181 行）
