...
- [ ] T005 (`FR-005`, `AC-002`): 截止日判据接线：`produce_partitions` 单元格过滤改用 `Admission.allows(pair, date)`；`_admitted_only` 迁入 `export_policy.admitted_only` 并按同一判据过滤判缺候选与增量继承的基线 `skipped`；`merge_partitions` 不改；`partitions.py`/`exporter.py` 净增 ≤0 — verify: `tests/integration/test_f011_export_journey.py`
