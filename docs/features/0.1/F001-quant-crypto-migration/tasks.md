...
- 浏览器目检（2026-09-12，AI 代人工执行）：Grafana 两看板截图确认渲染——Overview
  （6.31Mil 行/6 对分表/5 聚合/K 线图）与 Dry-run Monitoring（PnL/Trades=2/WinRate/
  快照新鲜度/PnL 历史/最新信号表）全部有数；实时采集器已启动（经代理 6/6 对、30s 周期），
  回填-实时间断档已定向回填补齐（25h/9018 行，聚合窗口一致），K 线延迟恢复至 <2 分钟。
