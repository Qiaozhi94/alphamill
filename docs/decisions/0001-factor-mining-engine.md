...
- [ ] **漏斗计数自冒烟第一天逐级入库**（PRD FR3.8/FR7.5）：生成 → 纯度门 → 因子证据/
  成本门 → 去重/多重检验 → 选择期 → 组合门并冻结 PortfolioDef → 留出门（内含
  样本量三级裁决）→ 最终确认 → 部署前复核，逐级记录绝对数与转化率；
...
PRD G3 已按 ADR-0008 修订：名义吞吐（原「稳定达到 ≥100 名义候选/周」）降级为漏斗分母与
能力标定，不再作为持续产出目标；M2 出口相应改为「单次 ≥50 候选自动评测 + 有效独立数/
来源分布/漏斗报告」。本 ADR 正文保留历史原文，其中两处计数判据的语义随之澄清：

- 「降级切换判据」中的候选计数（连续 2 周 <50、零候选过审计）是**引擎健康诊断**（产出
  归零 = 引擎坏），不是产出目标，继续有效；
- 「M2 冒烟验收」的漏斗计数义务照旧（FR3.8/FR7.5）。
