...
2. **新增不可变 `ResearchSnapshot`**：它是对多个有效 DatasetVersion 的内容寻址引用集合，
   ...
   ResearchSnapshot = {
     ...
     provenance: {created_at, artifact_path, member_manifest_paths,
                  symbol_map_path, universe_calendar_path}
   }
   ```
...
- symbol-map artifact 由 F002 内容寻址保存于 `lake/_metadata/symbol_maps/<digest>.csv`；调用方提供的
  universe/calendar JSON 由 `experiment_store` 规范化并保存于
  `reports/research_snapshots/_inputs/universe_calendars/<digest>.json`。ResearchSnapshot provenance
  引用这两个不可变 artifact，不能仅记录一个日后可能失去原文的摘要。
...
- 负面/义务：每次新的成员组合或 cutoff 都会多一个小型 manifest；symbol map、PIT universe 和
  ...
  文件或当前成员表补齐。F002 提供 symbol-map artifact；universe/calendar artifact 由调用 F007
  snapshot builder 的研究配置显式提供，不扩入 F002。
