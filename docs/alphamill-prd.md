...
| 资产 | 状态 | 位置 |
|---|---|---|
| TimescaleDB 数据湖 | ✅ 631 万行 1m OHLCV（6 个 OKX 永续对，2 年回补）+ 质量标记表 + 多周期连续聚合 | quant-crypto 仓库（待 M0 清算迁入本仓） |
...
- ❌ 不重建数据管道/数据湖（quant-crypto 数据资产整体迁入，见 [F001 migration-plan](features/0.1/F001-quant-crypto-migration/migration-plan.md)）
...
- **预注册三分支**（触发时三选一，分支判定与依据写入 experiment manifest）：
  1. **成本主导流失**：漏斗计数（FR6.3）显示成本档为最大流失级 → 执行**成本 pivot**（低换手 / maker 优先 / 拉长持有期方向）；
  2. **假设源枯竭**：IC 存活 ≈0 且 top-K 候选间 OOS PnL 高相关 → 执行**假设源 pivot**（转向种子库扩容 + 复盘循环假设多样化）；
  3. **≥2 个可信存活**（可信判定口径见 ADR-0003，≈69 笔）→ 继续主回路，但"继续"必须**书面再论证**（存活者质量 + 漏斗改善路径），不允许默认续期。
- **误杀防护（正控制）**：crypto 原生正控制——funding carry（与 FR2.5 同源数据）与主流币（BTC/ETH）截面动量——与挖掘候选**同期**跑全门禁。**正控制不过 = 评测台坏**（成本模型/宇宙/数据配置有误），不是论题死；先修台，再判分支。
- **跨窗复核**：检查点触发时的分支判定不得基于单一留出窗，须跨多个留出窗复核（与 ADR-0003 滚动留出路线联动）。
...
| S2 | 统一评测 | 因子 → RankIC / IC 衰减 / 分市场状态 / 分位数收益 / 成本硬过滤（taker+滑点，FR2.2）→ 存活者进回测与留出 |
...
| S5 | 部署 | 留出通过 → 信号写 feather 缓存 → 组合构建（协同池组合作为 meta-factor、Top-K 替代 Top1-only，见 `docs/decisions/0004-portfolio-construction.md`）→ Freqtrade 策略模板挂载 → dry-run → paper |
...
- FR1.3 宇宙扩容：复用 `discover_okx_swap_universe.py`，从 6 对扩到 30~50 对，质量标记流程沿用。**历史回填为显式工作流**（不隐藏在 FR1 开发内）：30~50 对需新增约 4600 万行 1m 数据（现有 631 万行），OKX 限速下预估 1~2 周 wall-clock；归属：项目所有者；与开发并行推进、不阻塞 FR1 其余交付，但回填完成前挖掘横截面 reward 噪声与评测台统计功效受限。
- FR1.4 Kronos 缓存与衍生品特征表纳入导出范围。

### FR2 因子工厂（Factor Factory）★ 核心

- FR2.1 **生成器**：至少两种可插拔后端——程序化（**AlphaGen RL 挖掘为主**，vendor 策略见 [ADR-0001](decisions/0001-factor-mining-engine.md)；GP/表达式枚举对照）与人工/LLM 辅助（论文因子、假设清单）。生成器只负责产出**因子定义**，不做评测。RL 奖励不得只含 RankIC：必须叠加成本惩罚项（按 FR2.5 成本模型）或评测前置的成本健全性预筛（成本后收益为负者不得进入存活名单），防止挖掘回路优化 IC 却批量产出死于成本墙的候选（quant-crypto 的死因不得被 ×20 吞吐量放大）。
- FR2.2 **统一评测台**：由 `kronos_rankic_eval.py` / `kronos_ic_decay_eval.py` 泛化而来——输入任意因子信号序列，输出 RankIC、IC 衰减曲线、分市场状态表现、分位数分组收益。评测台与生成器完全解耦。verdict 必须含**成本档硬过滤**：taker 费 + 滑点成本后收益为负的候选不得判 `promising`（IC 为正但成本后为负 → dead），promising/weak/dead 不得仅按 IC 判定。
...
- FR2.4 多重检验控制：候选通过数随时间累积时，显著性阈值需要校正（deflated significance / 白噪声基准对照）。**"一次试验"口径**：每条进入评测台的表达式（含被拒者）计 1 次试验；RL 每周产出数千条高相关表达式，试验总数（而非存活数）是 deflated significance 的分母。同时，候选间共享数据、奖励与协同池，有效独立假设数远低于名义试验数（约 5~10 个/周）——折减必须基于全部被评估表达式的计数，不得用存活者计数替代。
- FR2.5 **成本模型（一等公民）**：成本 = 交易费（taker/maker）+ 滑点 + **资金费率**。资金费率是永续合约的核心持有成本（每 8 小时结算，拥挤方向年化可达两位数），任何带持有期的信号必须计入；成本参数以 FR3.4 校准条款为准。
...
- FR5.3 部署流水线：留出 PASS → 自动生成策略配置 → dry-run 冒烟 → 人工确认挂载。部署形态遵循 [ADR-0004](decisions/0004-portfolio-construction.md)：协同池组合可作为 meta-factor 部署，Top-K 组合替代单因子 Top1-only。
...
| 成本现实 | 默认按 taker 0.05% + 滑点 + 资金费率建模（FR2.5）；参数以 dry-run 实测成交校准（FR3.4）；maker-only 策略显式声明 |
...
| 里程碑 | 内容 | 出口标准 | 预估 |
|---|---|---|---|
| **M0 资产清算迁移** | [F001 migration-plan](features/0.1/F001-quant-crypto-migration/migration-plan.md) 迁移清单：数据卷 + 采集器 + Kronos 上游化 + 风控/监控搬迁 | verify 全绿：行数对账 / Kronos /health / Freqtrade dry-run / 监控有数据 | 2-3 天 |
| **M1 数据桥 + 统一评测台** | FR1 开发交付 + FR2.2/2.3（kronos 评测器泛化）；FR1.3 历史回填（约 4600 万行 1m 数据，1~2 周 wall-clock）为独立并行工作流，归属：项目所有者，不计入开发排期，FR1 不声称包含全部回填 | 任意 feather 信号 → 一条命令出 RankIC/衰减/分状态/成本硬过滤报告；Kronos 现有缓存全部重新入库；回填完成前评测台先跑 6 对 | 1 周（开发）+ 1~2 周（回填，并行） |
...
| **M4 生产循环** | FR5 + FR6 + 宇宙扩容 | 每周 ≥100 候选进评测；首个候选通过 90 天留出（≥30 笔）→ dry-run；**论题检查点时钟随 M4 启动计时**（§2.4：10 周或 100 个有效独立候选，先到为准）；周报含漏斗计数并指认最大流失级（FR6.3） | 持续 |
...
| 信号仍过不了成本墙 | 成本三档敏感性为一级指标；成本硬过滤前置于评测台 verdict（FR2.2），资金费率入模（FR2.5）；maker-only/微择时 overlay 方向优先实验 |
| Vibe-Trading 学习成本 | 只接 4 个面：local loader、回测工具、quantlib、agent 复盘；不碰其实盘栈 |
| 吞吐量上来后人审瓶颈 | 评测台自动分诊：RankIC + 成本硬过滤 + 相关性查重先行过滤，人只看存活者 |
