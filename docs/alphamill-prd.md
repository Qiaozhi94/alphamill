# AlphaMill — 产品需求文档（PRD）

> 版本：v0.1 | 日期：2026-09-07 | 状态：草案
> 前置文档：tradingview-quant-research 仓库的 `research.md`（TradingView 调研）、quant-crypto 仓库的 `00-系统总体方案.md`（上一代系统）

---

## 一、背景与问题定义

### 1.1 quant-crypto 复盘（2026-05 ~ 2026-06）

上一代项目 quant-crypto 用 5 周时间完成了大量基础设施：

| 资产 | 状态 | 位置 |
|---|---|---|
| TimescaleDB 数据湖 | ✅ 631 万行 1m OHLCV（6 个 OKX 永续对，2 年回补）+ 质量标记表 + 多周期连续聚合 | quant-crypto 仓库（待 M0 清算迁入本仓） |
| 衍生品特征数据 | ✅ funding / mark-index basis / open interest 已回补 | `db/migrations/003` |
| Kronos 信号基建 | ✅ Kronos-base GPU 推理，RankIC > 0.05（自定"强有效"门槛） | `kronos-signal/` |
| 信号缓存模式 | ✅ 预计算 → feather → 策略按时间戳读取，**已通过无前视审计** | `freqtrade/user_data/kronos_cache/` |
| Freqtrade 执行 + 风控 | ✅ dry-run 运行；CircuitBreaker / CorrelationGuard / DrawdownGuard 验证触发 | `risk/` |
| 验证协议 | ✅ 选择期 + 最终 90 天留出门，正确拦截了所有过拟合候选 | `scripts/validate_*` |
| 监控 | ✅ Grafana + Prometheus + 信号质量监控（60m 收益回填、7 天滚动 IC） | `grafana/`, `prometheus/` |

**最终状态：`blocked_no_paper_candidate`** —— 所有候选（低频 Ridge/HGB、regime 增强、成本感知、衍生品 basis）全部死在最终留出期，Phase 5/6 未进入。

### 1.2 根因诊断

系统没有坏，坏掉的是 **alpha 假设吞吐量**：

- 5 周只测了 3 个信号族（Kronos 择时 / OHLCV+衍生品特征 Ridge/HGB / 阈值网格），每个族手工调数周；
- 量化中诚实 OOS 通过率通常 1~5%，需要每周 100+ 假设的测试量，当前每周约 5 个；
- 留出期样本量不足（69 笔交易的 FAIL/PASS 判定统计上不可信）；
- Kronos 5m 信号 RankIC > 0.05 但无法在成本后货币化——信号存在，**成本墙**未正面攻坚。

### 1.3 项目决策

1. **不重造轮子**：复用成熟开源。**Vibe-Trading**（HKUDS，20 万行 Python）作为研究回测与复盘总结的上层建筑；**Freqtrade** 作为交易执行基础设施。
2. **清算迁移 quant-crypto**：数据湖、信号缓存模式、验证门禁、风控三件套、监控栈一次性并入本仓（迁移清单见 [F001 migration-plan](features/0.1/F001-quant-crypto-migration/migration-plan.md)），原仓库归档废弃，只做必要适配。
3. **补齐唯一致命缺口**：因子自动挖掘模块（程序化批量生成 + 统一评测 + 生命周期管理）。

---

## 二、产品定位与目标

### 2.1 一句话定位

> AlphaMill（中文别名：淘沙）是一个**以因子假设吞吐量为核心指标**的加密量化研究系统：程序化批量挖掘因子 → 统一评测 → 严格留出门 → Freqtrade 执行，Vibe-Trading 作为研究/回测/复盘的上层建筑。

### 2.2 目标（含量化验收）

| ID | 目标 | 验收指标 |
|---|---|---|
| G1 | 因子假设吞吐量 ×20 | ≥ **100 个因子候选/周** 进入统一评测台（当前 ~5/周） |
| G2 | 验证协议不降级 | 100% 候选经过：选择期 → 最终 90 天留出（≥30 笔交易）才可进入 paper |
| G3 | Vibe-Trading 上层建筑接入 | ① Parquet→local loader 第二意见回测跑通；② agent 复盘工作流产出失败归因报告。**在 M3 time-box 内达成**；超时按 FR4.6 整体弃置时本目标随之降级（第二意见走按需手动复核路径），不阻塞主回路 |
| G4 | Freqtrade 执行基建继承 | 信号缓存契约兼容 kronos_cache；留出通过候选 ≤1 天内可挂 dry-run |
| G5 | 全链路可复现 | 100% 实验有 manifest（数据版本 + 代码版本 + 参数 + 结果摘要） |
| G6 | 论题存活判定有时钟（预注册止损） | 论题检查点触发器 = M4 启动后 10 周或 100 个有效独立候选进入评测（先到为准；独立性按留出期 OOS PnL 相关性聚类**测量**，不用名义计数）；触发时按 §2.4 预注册三分支执行——零存活不得以"门禁正确工作"叙事无限延期 |

### 2.3 非目标（明确排除）

- ❌ 不重建数据管道/数据湖（quant-crypto 数据资产整体迁入，见 [F001 migration-plan](features/0.1/F001-quant-crypto-migration/migration-plan.md)）
- ❌ 不替换 Freqtrade 执行引擎、不重建风控（继承 CircuitBreaker 等）
- ❌ 不做实盘：Phase 6 前置条件不变——必须有候选通过最终留出（quant-crypto 决策报告的规则原样继承）
- ❌ 不追求全市场覆盖：先做 OKX USDT 永续 30~50 对、1h/4h 低频
- ❌ 不用 TradingView 作为数据源（仅保留可选的可视化/Alert 层）

### 2.4 论题检查点（预注册止损线）

**动机（算术先行）**：G1 的 100 候选/周经 FR2.4 有效独立修正后仅 **5~10 个有效独立假设/周**；按诚实 OOS 通过率 1~5% 推算，期望存活节奏为 **每 2~20 周 1 个**（成本硬过滤还会进一步收窄分母）。该等待分布为几何/泊松型重尾：**论题即使成立，"连续多周零存活"也是常见事件**。没有预注册判读框架，零存活会被"门禁正确工作"（ADR-0003）无限合理化——这正是 quant-crypto 的运行模式，不得以更大规模重演。

- **触发器**：M4 启动后满 10 周，或累计 100 个有效独立候选进入评测（先到为准）。独立性不以名义试验计数，按留出期 OOS PnL 相关性聚类测量（聚类后每簇计 1 个有效独立假设）。
- **预注册三分支**（触发时三选一，分支判定与依据写入 experiment manifest）：
  1. **成本主导流失**：漏斗计数（FR6.3）显示成本档为最大流失级 → 执行**成本 pivot**（低换手 / maker 优先 / 拉长持有期方向）；
  2. **假设源枯竭**：IC 存活 ≈0 且 top-K 候选间 OOS PnL 高相关 → 执行**假设源 pivot**（转向种子库扩容 + 复盘循环假设多样化）；
  3. **≥2 个可信存活**（可信判定口径见 ADR-0003，≈69 笔）→ 继续主回路，但"继续"必须**书面再论证**（存活者质量 + 漏斗改善路径），不允许默认续期。
- **误杀防护（正控制）**：crypto 原生正控制——funding carry（与 FR2.5 同源数据）与主流币（BTC/ETH）截面动量——与挖掘候选**同期**跑全门禁。**正控制不过 = 评测台坏**（成本模型/宇宙/数据配置有误），不是论题死；先修台，再判分支。
- **跨窗复核**：检查点触发时的分支判定不得基于单一留出窗，须跨多个留出窗复核（与 ADR-0003 滚动留出路线联动）。

---

## 三、用户与核心场景

用户即项目所有者（研究员 + 交易员一体）。

| # | 场景 | 期望流程 |
|---|---|---|
| S1 | 批量挖掘因子 | 挖掘器（遗传/表达式/LLM）在 12~50 对数据上跑一代 → 产出 N 个因子定义 → 自动进评测台 |
| S2 | 统一评测 | 因子 → RankIC / IC 衰减 / 分市场状态 / 分位数收益 / 成本硬过滤（taker+滑点，FR2.2）→ 存活者进回测与留出 |
| S3 | 第二意见回测 | 存活候选同时投喂 Vibe-Trading 回测引擎（local loader 吃 Parquet），与 Freqtrade 级回测互相校验 |
| S4 | 失败复盘 | 留出失败的候选 → Vibe-Trading agent 读取实验 manifest + 报告 → 产出归因与新假设 |
| S5 | 部署 | 留出通过 → 信号写 feather 缓存 → 组合构建（协同池组合作为 meta-factor、Top-K 替代 Top1-only，见 `docs/decisions/0004-portfolio-construction.md`）→ Freqtrade 策略模板挂载 → dry-run → paper |
| S6 | 生命周期管理 | 已部署因子持续衰减监控（7 天滚动 IC 已有）→ 衰减超限自动降权/下线 |

---

## 四、功能需求

### FR1 数据桥（Data Bridge）

- FR1.1 定时任务将 TimescaleDB 核心表导出为 **Parquet 分区 + manifest.json**（来源、抓取时间、口径、版本号）。
- FR1.2 双口径：`close_adjusted`（算收益）与 `close_raw`（模拟成交）分离存储。
- FR1.3 宇宙扩容：复用 `discover_okx_swap_universe.py`，从 6 对扩到 30~50 对，质量标记流程沿用。**历史回填为显式工作流**（不隐藏在 FR1 开发内）：30~50 对需新增约 4600 万行 1m 数据（现有 631 万行），OKX 限速下预估 1~2 周 wall-clock；归属：项目所有者；与开发并行推进、不阻塞 FR1 其余交付，但回填完成前挖掘横截面 reward 噪声与评测台统计功效受限。
- FR1.4 Kronos 缓存与衍生品特征表纳入导出范围。

### FR2 因子工厂（Factor Factory）★ 核心

- FR2.1 **生成器**：至少两种可插拔后端——程序化（**AlphaGen RL 挖掘为主**，vendor 策略见 [ADR-0001](decisions/0001-factor-mining-engine.md)；GP/表达式枚举对照）与人工/LLM 辅助（论文因子、假设清单）。生成器只负责产出**因子定义**，不做评测。RL 奖励不得只含 RankIC：必须叠加成本惩罚项（按 FR2.5 成本模型）或评测前置的成本健全性预筛（成本后收益为负者不得进入存活名单），防止挖掘回路优化 IC 却批量产出死于成本墙的候选（quant-crypto 的死因不得被 ×20 吞吐量放大）。同理，奖励必须含**换手/交易频率维度**——奖励项叠加换手惩罚，或评测前置按 ≥30 笔/90 天可达性预筛；否则 IC 最优解机制性偏向低频/持仓型/成本盲表达式，被留出笔数门槛结构性拒绝——**优化目标与门禁筛选必须作用于同一种群**（种群错配以 FR6.3 漏斗计数监测）。
- FR2.2 **统一评测台**：由 `kronos_rankic_eval.py` / `kronos_ic_decay_eval.py` 泛化而来——输入任意因子信号序列，输出 RankIC、IC 衰减曲线、分市场状态表现、分位数分组收益。评测台与生成器完全解耦。verdict 必须含**成本档硬过滤**：taker 费 + 滑点成本后收益为负的候选不得判 `promising`（IC 为正但成本后为负 → dead），promising/weak/dead 不得仅按 IC 判定。
- FR2.3 **因子注册表**：元数据（定义、参数、数据依赖、所属生成器、评测结果摘要、生命周期状态 active/monitoring/decayed/disabled），带 IC 相关性查重（与现存因子 |IC 相关| > 0.99 拒绝，0.90~0.99 标记变体）。
- FR2.4 多重检验控制：候选通过数随时间累积时，显著性阈值需要校正（deflated significance / 白噪声基准对照）。**"一次试验"口径**：每条进入评测台的表达式（含被拒者）计 1 次试验；RL 每周产出数千条高相关表达式，试验总数（而非存活数）是 deflated significance 的分母。同时，候选间共享数据、奖励与协同池，有效独立假设数远低于名义试验数（约 5~10 个/周）——折减必须基于全部被评估表达式的计数，不得用存活者计数替代。
- FR2.5 **成本模型（一等公民）**：成本 = 交易费（taker/maker）+ 滑点 + **资金费率**。资金费率是永续合约的核心持有成本（每 8 小时结算，拥挤方向年化可达两位数），任何带持有期的信号必须计入；成本参数以 FR3.4 校准条款为准。

### FR3 验证门禁（原样继承 + 补强）

- FR3.1 选择期：验证集上选候选，规则先于数据确定。
- FR3.2 最终 90 天留出：≥30 笔交易才允许 FAIL/PASS 判定（解决 quant-crypto 的欠功效问题）。
- FR3.3 无前视审计：每个新因子必须通过独立逐 K 线重放器（复用 `independent_cross_backtest.py`）+ 信号缓存时间戳对齐检查。
- FR3.4 成本敏感性：taker/maker/零成本三档回测，成本后收益为主要指标。成本参数（费率/滑点/资金费率）必须以 quant-crypto dry-run 的实际成交记录（现有资产）校准，不得直接采用交易所牌价。

### FR4 Vibe-Trading 接入（研究上层建筑）

- FR4.1 Parquet 湖接入 Vibe-Trading `local` loader（`local:` 前缀，禁止静默回退网络源）。
- FR4.2 第二意见回测：存活候选在 Vibe-Trading 引擎（含 quantlib、MC/Bootstrap/Walk-Forward 验证）复跑，与 Freqtrade 级回测差异需可解释。
- FR4.3 agent 复盘工作流：输入失败实验的 manifest + 报告 → 产出归因分析 + 新特征假设清单（人审后进入 FR2.1 生成器队列）。
- FR4.4 因子生命周期：参考其 SDM（注册/状态/衰减扫描）模式，对接 FR2.3 注册表。
- FR4.5 **边界**：不使用 Vibe-Trading 的实盘交易栈（与现有风控重复）；不作为行情数据源。
- FR4.6 **可弃置声明**：Vibe-Trading 是系统里最不承重的组件——M3 整体 time-box（1 周），超时或吞噬主回路排期时**整体可弃置**（弃置决定与原因记入实验台账/复盘，不算里程碑失败）。FR4.2 第二意见的正确性保险功能有廉价降级路径：**按需手动复核**——怀疑评测台 bug 时，在候选子集上用第二实现（Vibe 回测或独立重放器）重算若干门禁决策并比对。该降级路径不阻塞主链路、不依赖 Vibe-Trading 存活；FR4.3 agent 复盘的价值并入种子库/假设清单管理，不构成保留 M3 的强制理由。

### FR5 信号缓存与部署

- FR5.1 信号缓存契约与 `kronos_cache` 兼容：feather 按日分片，列 = (timestamp, pair, signal_value, [confidence])，时间戳为信号确认时刻。
- FR5.2 Freqtrade 策略模板：读缓存 → 入场/出场条件 → `confirm_trade_entry` 风控钩子，模板化使新因子接入 ≤ 半天。
- FR5.3 部署流水线：留出 PASS → 自动生成策略配置 → dry-run 冒烟 → 人工确认挂载。部署形态遵循 [ADR-0004](decisions/0004-portfolio-construction.md)：协同池组合可作为 meta-factor 部署，Top-K 组合替代单因子 Top1-only。

### FR6 实验台账与复盘

- FR6.1 每个实验写 manifest：数据快照版本、代码 commit、参数、评测结果、结论（PASS/FAIL + 原因）。挖掘类实验的 manifest 必须额外记录随机种子与确定性开关（torch deterministic mode / cudnn deterministic 等）——否则 G5 的全链路复现对挖掘步骤不成立。manifest 同时记录 **cost_model_version**（成本参数集版本，与数据版本并列）：成本模型变更后的候选重定价依赖评测台 trade_log_summary（架构 §4.2），重算而不重跑。
- FR6.2 失败案例库：留出失败候选的归因报告沉淀，供 S4 场景与后续生成器参考。
- FR6.3 **漏斗计数（逐级入库，从 M2 冒烟第一天开始）**：挖掘 → 评测管线输出逐级漏斗计数并写入实验台账：`挖掘产出 → IC 通过 → 成本硬过滤通过 → 笔数门槛通过 → 留出通过`，逐级绝对数与相邻转化率入库。漏斗是**优化目标与门禁种群错配的可视化器**（FR2.1）：某级转化率异常塌陷即该级在系统性筛掉种群。M4 周报必须给出当周漏斗并**指认最大流失级**——流失最重的那一级是论题检查点（§2.4）分支判定的强制输入。

---

## 五、非功能需求

| 类别 | 要求 |
|---|---|
| 可复现 | 任一历史结果可从 manifest 从零复现（对齐 research.md 第八节验收标准） |
| 无前视 | 所有因子定义过 AST 纯度检查（借用 Vibe-Trading Alpha Zoo 的 lookahead 哨兵思路）；评测/回测分离数据窗口 |
| 成本现实 | 默认按 taker 0.05% + 滑点 + 资金费率建模（FR2.5）；参数以 dry-run 实测成交校准（FR3.4）；maker-only 策略显式声明 |
| 安全 | 实盘前置条件不变；API key 仅存 `.env`/系统凭据库；Vibe-Trading 仅本机回环运行 |
| 资源 | 复用现有单机（RTX 4060 Laptop GPU）；挖掘/评测为 CPU 密集可夜间批跑 |
| 里程碑纪律 | 每个里程碑出口有可运行验证脚本（沿用 quant-crypto verify 习惯） |

---

## 六、里程碑

| 里程碑 | 内容 | 出口标准 | 预估 |
|---|---|---|---|
| **M0 资产清算迁移** | [F001 migration-plan](features/0.1/F001-quant-crypto-migration/migration-plan.md) 迁移清单：数据卷 + 采集器 + Kronos 上游化 + 风控/监控搬迁 | verify 全绿：行数对账 / Kronos /health / Freqtrade dry-run / 监控有数据 | 2-3 天 |
| **M1 数据桥 + 统一评测台** | FR1 开发交付 + FR2.2/2.3（kronos 评测器泛化）；FR1.3 历史回填（约 4600 万行 1m 数据，1~2 周 wall-clock）为独立并行工作流，归属：项目所有者，不计入开发排期，FR1 不声称包含全部回填 | 任意 feather 信号 → 一条命令出 RankIC/衰减/分状态/成本硬过滤报告；Kronos 现有缓存全部重新入库；回填完成前评测台先跑 6 对；**另附论题探针三件套**（见下） | 1 周（开发）+ 1~2 周（回填，并行） |
| **M2 因子挖掘管线** | FR2.1 程序化生成器（**AlphaGen vendor + 现代栈冒烟 → feather→tensor 适配器**）+ FR2.4 + FR3.3 | 冒烟：现代栈跑通 1 个 PPO epoch 产出因子；单次挖掘 ≥50 候选 → 自动评测 → ≥5 个进选择期；无前视审计通过；**漏斗计数自冒烟第一天逐级入库**（FR6.3 / ADR-0001 补强节） | 2 周（冒烟 1-2 天 + 适配器 ~1 周） |
| **M3 Vibe-Trading 接入** | FR4.1/4.2/4.3 | Parquet→local loader 回测跑通并与 Freqtrade 级回测出对比报告；agent 复盘工作流产出首份归因报告。**time-box 可弃置**（FR4.6）：超时即整体弃置，不拖累主回路 | 1 周（time-box，可弃置） |
| **M4 生产循环** | FR5 + FR6 + 宇宙扩容 | 每周 ≥100 候选进评测；首个候选通过 90 天留出（≥30 笔）→ dry-run；**论题检查点时钟随 M4 启动计时**（§2.4：10 周或 100 个有效独立候选，先到为准）；周报含漏斗计数并指认最大流失级（FR6.3） | 持续 |

**M1 论题探针三件套（约 1 天工作量）**：M1 天然是论题的首个方向性证据来源（Kronos 信号 + 种子公式过含成本硬过滤的新评测台），不应被框定为纯管道工作、把首个方向性证据推迟到 M4。出口标准在表列三项外追加：

1. **crypto 原生正控制报告**：funding carry 与 BTC/ETH 截面动量必须通过成本硬过滤——不过 = 成本模型或宇宙配置有误（§2.4 正控制防护在 M1 的提前落地，避免正控制缺位拖到检查点才发现台坏）；
2. **种子库 OOS PnL 相关性矩阵**：GTJA191 子集 + Kronos 信号的留出期 PnL 相关性聚类，把"有效独立假设 ≈5~10/周"（FR2.4）从文档假设变成**测量基线**——AlphaGen 产出必须打败它（论题情景"假设源多样性才是真约束"的直接探针）；
3. **成本分解报告**：taker 费 / 资金费率 / 滑点各占毛 alpha 消耗的份额——为 §2.4 成本 pivot 分支提供定量输入。

---

## 七、风险与缓解

| 风险 | 缓解 |
|---|---|
| 程序化挖掘（RL/GP）大量产出伪因子（泄漏/幸存者偏差/RL reward hacking） | AST 纯度门 + 独立重放审计 + 白噪声对照 + 多重检验校正（FR2.4/FR3.3） |
| 扩宇宙后数据质量下降 | 沿用 quant-crypto 质量标记表与缺失告警流程 |
| 信号仍过不了成本墙 | 成本三档敏感性为一级指标；成本硬过滤前置于评测台 verdict（FR2.2），资金费率入模（FR2.5）；maker-only/微择时 overlay 方向优先实验 |
| Vibe-Trading 学习成本 | 只接 4 个面：local loader、回测工具、quantlib、agent 复盘；不碰其实盘栈 |
| 吞吐量上来后人审瓶颈 | 评测台自动分诊：RankIC + 成本硬过滤 + 相关性查重先行过滤，人只看存活者 |
